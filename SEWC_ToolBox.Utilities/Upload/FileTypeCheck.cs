...
namespace SEWC_ToolBox.Utilities.Upload
{
    ...
    {
        private readonly static Dictionary<string, List<byte[]>> FILEEXTS = new Dictionary<string, List<byte[]>>
                    {
        ...
        private static Dictionary<string, List<byte[]>> GetFileExtension(string[] exts)
        {
            var res = FILEEXTS.Where(a => exts.FirstOrDefault(b => a.Key.Equals(b,StringComparison.InvariantCultureIgnoreCase)) != null).ToDictionary(a => a.Key, a => a.Value);
        ...
        /// <param name="fileData">文件头字节数组</param>
        /// <param name="checkExts">要验证的文件后缀集合</param>
        /// <returns></returns>
        public static bool IsValidFileExtension(string fileName, byte[] fileData, params string[] checkExts)
        {
            ...
            var exts = GetFileExtension(checkExts);
            if (!exts.ContainsKey(ext))
            {
                return false;
            }
            List<byte[]> sig = exts[ext];
